...
function setup() {
  ...
  radius = width * 0.45; // veľkosť kruhu
...
function draw() {
  ...
  // pridávaj nové vlákna zo stredu
  if (frameCount % 40 === 0) {
    for (let i = 0; i < 15; i++) {
      fibers.push(new Fiber(center.x, center.y));
    }
...
class Fiber {
  constructor(x, y) {
    ...
    this.weight = random(2, 5); // hrubšie linky
  ...
  display() {
    // Kreslí len ak bod leží vo vnútri kruhu
